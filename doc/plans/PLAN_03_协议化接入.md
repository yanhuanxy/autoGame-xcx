...
> 状态：**主线方向，待启动**（2026-07-27 owner 拍板为 autogame 长期主线）。首选第一步跑 §3.1-3.2 的 mitmproxy 抓包最小验证，验证协议直连路径成本后再决定走协议直连还是 JS 注入。`mitmproxy` 为本方向依赖。
> 项目：`wechat-link-autogame-xcx`
> 方向：从图像识别升级为协议直连 + JS 注入，针对微信小程序内的 H5 + Canvas 渲染游戏
> 预计周期：7 阶段 = 约 3~6 周
> 前置依赖：方向 1 完成（包结构稳定）

> ⚠️ **合规红线（硬约束）**：协议化代码仅用于个人研究，**不商业化、不公开分发、绝不随 wechat-ilink-bot 开源仓分发**（bot 是 MIT 开源，协议逆向代码必须留在本 autogame 私有仓）。bot 仓只保留 MCP 客户端契约，不接触任何协议实现。详见 §1.2 法律风险。
